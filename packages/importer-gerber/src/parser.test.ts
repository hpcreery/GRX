import { FeatureTypeIdentifier } from "@grx/artwork-format/types";
import { expect } from "@std/expect";
import { MACRO_PRIMITIVE, MACRO_VARIABLE } from "./constants";
import { GerberLexer, GerberToTreeVisitor, parse, parser } from "./parser";
import fullCircleGerber from "../testdata/gerbers/arc-strokes/full-circle.gbr" with {
  type: "text",
};
import blockApertureNestedGerber from "../testdata/gerbers/block-apertures/nested.gbr" with {
  type: "text",
};
import outlinePrimitiveGerber from "../testdata/gerbers/macro-primitives/outline-primitive.gbr" with {
  type: "text",
};
import thermalPrimitiveGerber from "../testdata/gerbers/macro-primitives/thermal-primitive.gbr" with {
  type: "text",
};
import vectorPrimitiveGerber from "../testdata/gerbers/macro-primitives/vector-primitive.gbr" with {
  type: "text",
};
import regionWithLinesGerber from "../testdata/gerbers/regions/region-with-lines.gbr" with {
  type: "text",
};
import stepRepeatMultiPolarityGerber from "../testdata/gerbers/step-repeats/multi-polarity.gbr" with {
  type: "text",
};
import stepRepeatGerber from "../testdata/gerbers/step-repeats/one-polarity.gbr" with {
  type: "text",
};
import singleSegmentGerber from "../testdata/gerbers/strokes/circle-tool-single-segment.gbr" with {
  type: "text",
};
import circleToolZeroLengthGerber from "../testdata/gerbers/strokes/circle-tool-zero-length.gbr" with {
  type: "text",
};
...
Deno.test("parses a block aperture fixture and emits geometry", () => {
  const image = parseAndPlot(blockApertureNestedGerber);
  expect(image.length).toBeGreaterThan(0);
});

Deno.test("parses a single-segment stroke and emits a line", () => {
  const image = parseAndPlot(singleSegmentGerber);
  const lines = image.filter((shape) =>
    shape.type === FeatureTypeIdentifier.LINE
  );
  expect(lines.length).toBeGreaterThan(0);
});

Deno.test("parses a zero-length stroke fixture", () => {
...
Deno.test("parses full-circle arc fixture and emits arcs", () => {
  const image = parseAndPlot(fullCircleGerber);
  const arcs = image.filter((shape) =>
    shape.type === FeatureTypeIdentifier.ARC
  );
  expect(arcs.length).toBeGreaterThan(0);
});

Deno.test("parses region fixture and emits surfaces", () => {
  const image = parseAndPlot(regionWithLinesGerber);
  const surfaces = image.filter((shape) =>
    shape.type === FeatureTypeIdentifier.SURFACE
  );
  expect(surfaces.length).toBeGreaterThan(0);
});

Deno.test("parses step-repeat fixture and emits step-and-repeat", () => {
  const image = parseAndPlot(stepRepeatGerber);
  const repeats = image.filter((shape) =>
    shape.type === FeatureTypeIdentifier.STEP_AND_REPEAT
  );
  expect(repeats.length).toBeGreaterThan(0);
});

Deno.test("parses multi-polarity step-repeat fixture", () => {
...
Deno.test("parses macro aperture fixtures and emits geometry", () => {
  const image = parseAndPlot(vectorPrimitiveGerber);
  expect(image.length).toBeGreaterThan(0);
});

Deno.test("parses AM variable expressions into macro blocks", () => {
...
M02*`;
  ...
  const variableBlocks =
    macroNode?.filter((child) => child.type === MACRO_VARIABLE) ?? [];
...
Deno.test("parses outline and thermal AM primitive families", () => {
  const outlineMacro =
    parseAndVisit(outlinePrimitiveGerber).visitor.macroDefinitions["OUTLINE"];
  const thermalMacro =
    parseAndVisit(thermalPrimitiveGerber).visitor.macroDefinitions["THERMAL"];

  expect(outlineMacro).toBeDefined();
  expect(thermalMacro).toBeDefined();

  const outlinePrimitive = outlineMacro?.find((child) =>
    child.type === MACRO_PRIMITIVE
  );
  const thermalPrimitive = thermalMacro?.find((child) =>
    child.type === MACRO_PRIMITIVE
  );
...
Deno.test("supports modal D01 operation when operation code is omitted", () => {
...
Deno.test("supports modal D03 flash at current point", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
%ADD10C,0.01*%
D10*
...
M02*`;
  ...
  const { image } = parseAndVisit(gerber);
  const pads = image.filter((shape) =>
    shape.type === FeatureTypeIdentifier.PAD
  );
...
Deno.test("supports inline interpolation command style", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
%ADD10C,0.01*%
D10*
...
Deno.test("parses FS trailing-zero absolute form (FSTA)", () => {
...
// LR Rotate
// LS Scale
Deno.test("applies LP/LM/LR/LS transforms to flashed pads", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
%ADD10C,0.01*%
%LPC*%
...
Deno.test("applies and deletes file/aperture/object attributes", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
...
// cleared.
Deno.test("deletes all attribute dictionaries with TD and no name", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
...
Deno.test("parses deprecated command families and records warnings where expected", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
G70*
G71*
...
M00*`;
  ...
  expect(messageIncludes(visitor, "Negative images are not fully supported"))
    .toBe(true);
});

Deno.test("records a mismatch error for step-repeat close without matching open", () => {
  const gerber = `%SR*%
M02*`;
  ...
  expect(lexingErrors).toHaveLength(0);
  expect(parserErrors).toHaveLength(0);
  expect(messageIncludes(visitor, "Mismatched step and repeat close command"))
    .toBe(true);
});

Deno.test("parses deprecated macro primitive codes into macro definitions", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
%AMDEPRIMS*
2,1,0.1,0,0,1,0*
...
M02*`;
  ...
  const primitiveCodes = (macro ?? []).filter((node) =>
    node.type === MACRO_PRIMITIVE
  ).map((node) => node.code);
...
Deno.test("auto-finalizes parsing when M02 is omitted", () => {
  const gerber = `%FSLAX24Y24*%
%MOIN*%
%ADD10C,0.01*%
D10*
...
Deno.test("reports parser or lexer issues for malformed syntax", () => {
...
Deno.test.ignore(
  "should validate strict spec constraints for deprecated commands ordering and single-use semantics",
  () => {},
);
Deno.test.ignore(
  "should validate unknown command warning behavior mandated by spec conformance rules",
  () => {},
);
