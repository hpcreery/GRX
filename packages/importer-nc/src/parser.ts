...
import { type AttributesType, type Binary, FeatureTypeIdentifier, type Units } from "@grx/artwork-format/types"
...
export class NCToShapesVisitor extends BaseCstVisitor {
  ...
  toolChange(ctx: Cst.ToolChangeCstChildren): void {
    ...
    const compensationIndexTrimmed = String(Number(compensationIndex.slice(1, 3)))
    const toolNumberTrimmed = String(Number(tool.slice(1, 3)))
    if (!this.toolStore[toolNumberTrimmed]) {
      this.errors.push(`Tool ${tool} not defined before use. ${ctx.T[0].startLine}:${ctx.T[0].startColumn}`)
    }
    if (compensationIndex !== "" && !this.compensationStore[compensationIndexTrimmed]) {
      this.errors.push(`Compensation index ${compensationIndex} not defined before use. ${ctx.T[0].startLine}:${ctx.T[0].startColumn}`)
    }
    this.state.currentTool = this.toolStore[toolNumberTrimmed] ?? defaultTool
    if (compensationIndex !== "") {
      this.state.cutterCompensation = this.compensationStore[compensationIndexTrimmed] ?? 0
    } else {
  ...
  stopForInspect(_ctx: Cst.StopForInspectCstChildren): void {
    this.result.push(
      new Shapes.DatumText({
        ...
        x: this.state.x,
        y: this.state.y,
        text: "Stop for Inspect",
        attributes: {
          type: "stop for inspect",
          chainIndex: `${this.state.chainIndex}`,
        },
      }),
    )
  ...
  retractWithClamping(_ctx: Cst.RetractWithClampingCstChildren): void {
    ...
    this.result.push(
      new Shapes.DatumText({
        ...
        x: this.state.x,
        y: this.state.y,
        text: "Retract with clamping",
        attributes: { type: "retract with clamping" },
      }),
    )
  }
  ...
  longOperatorMessage(ctx: Cst.LongOperatorMessageCstChildren): void {
    this.result.push(
      new Shapes.DatumText({
        ...
        x: this.state.x,
        y: this.state.y,
        text: ctx?.Text?.[0]?.image || "",
        attributes: { type: "long operator message" },
      }),
    )
  }
  ...
  operatorMessage(_ctx: Cst.OperatorMessageCstChildren): void {
    this.result.push(
      new Shapes.DatumText({
        ...
        x: this.state.x,
        y: this.state.y,
        text: _ctx?.Text?.[0]?.image || "",
        attributes: { type: "operator message" },
      }),
    )
  }
