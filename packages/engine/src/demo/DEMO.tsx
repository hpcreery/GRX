...
function DemoApp(): JSX.Element {
  ...
  React.useEffect(() => {
    ...
    DataInterface.create_layer(project, "asdf")
    DataInterface.create_step_layer_artwork(project, step1, "asdf", [
      new Shapes.Pad({
        x: 0,
        y: 5,
        rotation: 0,
...
function REGLStatsWidget(props: { renderer: Renderer }): JSX.Element {
  ...
  const update = async (): Promise<void> => {
    ...
    const renderTime = await (await props.renderer.engine.timer).renderTimeMilliseconds
