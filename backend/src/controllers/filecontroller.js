...
module.exports = {
  ...
  createDir(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .send({ Status: 'Error', Message: 'Error Creating Directory' });
    }
  ...
  deleteFile(req, res) {
    ...
    try {
      fs.unlinkSync(
        path.join(db, req.query.job, 'gerbers', req.query.filename)
      );
    } catch (err) {
      console.log(err);
  ...
  deleteJob(req, res) {
    try {
      ...
      res
        .status(500)
        .send({ Status: 'Error', Message: 'Error Removing Directory' });
    }
    res.status(200).send({ Status: 'Done' });
  },
};
