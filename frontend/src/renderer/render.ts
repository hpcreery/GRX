  ...
  public drawPath(segments: PathSegment[]): this {
    for (const [index, next] of segments.entries()) {
      ...
      if (next.type === LINE) {
        ...
        const startTangent = [start[0] - center[0], start[1] - center[1]]
        const endTangent = [end[0] - center[0], end[1] - center[1]]
        this.arcTo(start[0]*100, start[1]*100, end[0]*100, end[1]*100, radius*100);
      }
    }
    return this;
  }
