...
class Renderer extends Component {
  ...
  addLayer = (layer, visible) => {
    ...
    if (svgElement.id === 'back') {
      svgChildElement.style.bottom = `calc(${
        height + originy
      }in * var(--svg-scale))`;
      svgChildElement.style.left = `calc(${
        originx + width
      }in * var(--svg-scale))`;
    } else {
      svgChildElement.style.bottom = `calc(${
    ...
    // svgChildElement.onclick(e => this.getClicked3DPoint(e))
    //this.captureMouse2(svgChildElement);
  ...
  captureMouse2 = (obj) => {
    obj.addEventListener('mousemove', (event) => {
      ...
      console.log('(' + (pt.x/prematrix[0])/95.2 + ', ' + (pt.y/prematrix[0])/95.2 + ')');
    });
  };
