...
float pullSymbolParameter_1(int offset, int SymNum) {
...
float merge_0(float d1, float d2) {
  return min(d1, d2);
}
...
vec2 rotateCCW_0(vec2 p, float angle) {
  mat2 m = mat2(cos(angle), sin(angle), -sin(angle), cos(angle));
  return p * m;
}

vec2 rotateCW_0(vec2 p, float angle) {
  mat2 m = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
  return p * m;
}

mat2 rotateCCW_0(float angle) {
  return mat2(cos(angle), sin(angle), -sin(angle), cos(angle));
}

mat2 rotateCW_0(float angle) {
  return mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
}

vec2 translate_0(vec2 p, vec2 t) {
  return p - t;
}
...
float boxDist_0(vec2 p, vec2 size) {
  size /= 2.0;
  vec2 d = abs(p) - size;
  return min(max(d.x, d.y), 0.0) + length(max(d, 0.0));
}
...
float verticalHexagonDist(vec2 p, vec2 size, float radius) {
  ...
  return max(dist, boxDist_0(p, size));
}
...
float butterflyDist(vec2 p, float dist) {
  ...
  float ulSquareDist = boxDist_0(p - vec2(-0.5 * dist, 0.5 * dist), vec2(dist, dist));
  float lrSquareDist = boxDist_0(p - vec2(0.5 * dist, -0.5 * dist), vec2(dist, dist));
...
float squareButterflydist(vec2 p, float dist) {
  float ul = boxDist_0(p - vec2(-0.5 * dist, 0.5 * dist), vec2(dist, dist));
  float lr = boxDist_0(p - vec2(0.5 * dist, -0.5 * dist), vec2(dist, dist));
  float d = min(ul, lr);
  return d;
}
...
float squareThermalDist(in vec2 p, in float od, in float id, in float angle, in float num_of_spokes, in float gap) {
  float outersquare = boxDist_0(p, vec2(od, od));
  float innersquare = boxDist_0(p, vec2(id, id));
...
float boxThermalDist(in vec2 p, in float width, in float height, in float angle, in float num_of_spokes, in float gap, in float line_width) {
  float outerbox = boxDist_0(p, vec2(width, height));
  float innerbox = boxDist_0(p, vec2(width - line_width * 2.0, height - line_width * 2.0));
  ...
  if (mod(angle, 45.0) == 0.0) {
    ...
    if (width > height && abs(offset.x) < abs(p.x)) {
      d = max(d, spokeDist(translate_0(p, offset), angle, num_of_spokes, gap));
    }
...
float boxThermalOpenCornersDist(in vec2 p, in float width, in float height, in float angle, in float num_of_spokes, in float gap, in float line_width) {
  ...
  if (mod(angle, 45.0) == 0.0) {
    ...
    float topBox = boxDist_0(translate_0(p, topLocation), vec2(topWidth, line_width));
    float botBox = boxDist_0(translate_0(p, botLocation), vec2(botWidth, line_width));
    ...
    float leftBox = boxDist_0(translate_0(p, leftLocation), vec2(line_width, leftHeight));
    float rightBox = boxDist_0(translate_0(p, rightLocation), vec2(line_width, rightHeight));

    float tb = merge_0(topBox, botBox);
    float lr = merge_0(leftBox, rightBox);
    return merge_0(lr, tb);
  }
...
float lineThermalDist(in vec2 p, in float outerDia, in float innerDia, in float angle, in float num_of_spokes, in float gap) {
  ...
  vec2 topLocation = vec2(corners.y * (width / 2.0 - topWidth / 2.0) - corners.x * (width / 2.0 - topWidth / 2.0), height / 2.0 - line_width / 2.0);
  vec2 botLocation = vec2(corners.z * (width / 2.0 - botWidth / 2.0) - corners.w * (width / 2.0 - botWidth / 2.0), -height / 2.0 + line_width / 2.0);
  float topLine = roundBoxDist(translate_0(p, topLocation), vec2(topWidth, line_width), line_width / 2.0);
  float botLine = roundBoxDist(translate_0(p, botLocation), vec2(botWidth, line_width), line_width / 2.0);
  ...
  float leftLine = roundBoxDist(translate_0(p, leftLocation), vec2(line_width, leftHeight), line_width / 2.0);
  float rightLine = roundBoxDist(translate_0(p, rightLocation), vec2(line_width, rightHeight), line_width / 2.0);

  float tb = merge_0(topLine, botLine);
  float lr = merge_0(leftLine, rightLine);
  return merge_0(lr, tb);
}
    ...
    if (mod(angle, 180.0) == 0.0 && num_of_spokes == 2.0) {
      if (rounded == 1.0) {
        if (abs(offset.x) < abs(p.x)) {
          d = roundedRoundThermalDist(translate_0(p, offset), height, height - line_width * 2.0, angle, num_of_spokes, gap);
        }
      } else {
        d = max(d, spokeDist(p, angle, num_of_spokes, gap));
      }
      ...
      if (rounded == 1.0) {
        if ((line_width + gap) / 2.0 > abs(p.x)) {
          d = circleDist(translate_0(abs(p), vec2((line_width + gap) / 2.0, (height - line_width) / 2.0)), line_width / 2.0);
        }
      } else {
        d = max(d, spokeDist(p, angle, num_of_spokes, gap));
      }
      ...
      if (rounded == 1.0) {
        ...
        if (abs(offset.x) < abs(p.x)) {
          d = roundThermalDist(translate_0(p, offset), height, height - line_width * 2.0, angle, num_of_spokes, gap);
        }
      }
      return d;
    } else {
    ...
    if (mod(angle, 180.0) == 0.0 && num_of_spokes == 2.0) {
      if (rounded == 1.0) {
        if ((line_width + gap) / 2.0 > abs(p.y)) {
          d = circleDist(translate_0(abs(p), vec2((width - line_width) / 2.0, (line_width + gap) / 2.0)), line_width / 2.0);
        }
      } else {
        d = max(d, spokeDist(p, angle, num_of_spokes, gap));
      }
      ...
      if (rounded == 1.0) {
        if (abs(offset.y) < abs(p.y)) {
          d = roundedRoundThermalDist(translate_0(p, offset), width, width - line_width * 2.0, angle, num_of_spokes, gap);
        }
      } else {
        d = max(d, spokeDist(p, angle, num_of_spokes, gap));
      }
      ...
      if (rounded == 1.0) {
        ...
        if (abs(offset.y) < abs(p.y)) {
          d = roundedRoundThermalDist(translate_0(p, offset), width, width - line_width * 2.0, angle + 90.0, num_of_spokes - 2.0, gap);
        }
      } else {
        d = max(d, spokeDist(p, angle, num_of_spokes, gap));
      }
      ...
      if (rounded == 1.0) {
        ...
        if (abs(offset.y) < abs(p.y)) {
          d = roundThermalDist(translate_0(p, offset), width, width - line_width * 2.0, angle, num_of_spokes, gap);
        }
      }
      return d;
    } else {
...
float moireDist(vec2 p, float ring_width, float ring_gap, float num_rings, float line_width, float line_length, float angle) {
  ...
  float lines_edge = boxDist_0(p * rotateCW_0(-radians(angle)), vec2(line_length, line_length));
...
float pullSymbolParameter_0(int offset, int SymNum) {
  vec2 texcoord = (vec2(float(offset), SymNum) + 0.5) / u_SymbolsTextureDimensions;
  vec4 pixelValue = texture2D(u_SymbolsTexture, texcoord);
  return pixelValue.x;
}
...
float drawShape(vec2 FragCoord, int SymNum) {
  ...
  float t_Symbol = pullSymbolParameter_0(u_Parameters.symbol, SymNum);
  float t_Width = pullSymbolParameter_0(u_Parameters.width, SymNum);
  float t_Height = pullSymbolParameter_0(u_Parameters.height, SymNum);
  float t_Corner_Radius = pullSymbolParameter_0(u_Parameters.corner_radius, SymNum);
  float t_Corners = pullSymbolParameter_0(u_Parameters.corners, SymNum);
  float t_Outer_Dia = pullSymbolParameter_0(u_Parameters.outer_dia, SymNum);
  float t_Inner_Dia = pullSymbolParameter_0(u_Parameters.inner_dia, SymNum);
  float t_Line_Width = pullSymbolParameter_0(u_Parameters.line_width, SymNum);
  float t_Line_Length = pullSymbolParameter_0(u_Parameters.line_length, SymNum);
  float t_Angle = pullSymbolParameter_0(u_Parameters.angle, SymNum);
  float t_Gap = pullSymbolParameter_0(u_Parameters.gap, SymNum);
  float t_Num_Spokes = pullSymbolParameter_0(u_Parameters.num_spokes, SymNum);
  float t_Round = pullSymbolParameter_0(u_Parameters.round, SymNum);
  // float t_Cut_Size = pullSymbolParameter(u_Parameters.cut_size);
  float t_Ring_Width = pullSymbolParameter_0(u_Parameters.ring_width, SymNum);
  float t_Ring_Gap = pullSymbolParameter_0(u_Parameters.ring_gap, SymNum);
  float t_Num_Rings = pullSymbolParameter_0(u_Parameters.num_rings, SymNum);
  ...
  if (t_Symbol == u_Shapes.Round || t_Symbol == u_Shapes.Hole) {
    ...
    dist = boxDist_0(FragCoord.xy, vec2(t_Width, t_Height));
    if (t_Inner_Dia != 0.0) {
      float hole = -1.0 * circleDist(FragCoord.xy, t_Inner_Dia / 2.0);
      dist = max(dist, hole);
    }
    ...
    float InnerSquare = boxDist_0(FragCoord.xy, vec2(t_Inner_Dia, t_Inner_Dia));
    float OuterSquare = boxDist_0(FragCoord.xy, vec2(t_Outer_Dia, t_Outer_Dia));
    dist = substract(InnerSquare, OuterSquare);
  } else if (t_Symbol == u_Shapes.SquareRound_Donut) {
    float InnerCircle = circleDist(FragCoord.xy, t_Inner_Dia / 2.0);
    float OuterCircle = boxDist_0(FragCoord.xy, vec2(t_Outer_Dia, t_Outer_Dia));
    dist = substract(InnerCircle, OuterCircle);
    ...
    float InnerRect = boxDist_0(FragCoord.xy, vec2(t_Width - t_Line_Width * 2.0, t_Height - t_Line_Width * 2.0));
    float OuterRect = boxDist_0(FragCoord.xy, vec2(t_Width, t_Height));
    ...
    float outerbox = boxDist_0(FragCoord.xy, vec2(t_Outer_Dia, t_Outer_Dia));
    float innercircle = circleDist(FragCoord.xy, t_Inner_Dia / 2.0);
    ...
    dist = regularPolygonDist(FragCoord.xy, t_Outer_Dia / 2.0, int(t_Corners));
    if (t_Line_Width != 0.0) {
      float inner = -1.0 * regularPolygonDist(FragCoord.xy, (t_Outer_Dia / 2.0) - t_Line_Width, int(t_Corners));
      dist = max(dist, inner);
    }
    if (t_Inner_Dia != 0.0) {
      float hole = -1.0 * circleDist(FragCoord.xy, t_Inner_Dia / 2.0);
...
void main() {
  ...
  float t_Outer_Dia = pullSymbolParameter_1(u_Parameters.outer_dia, int(v_SymNum));
  float t_Width = pullSymbolParameter_1(u_Parameters.width, int(v_SymNum));
