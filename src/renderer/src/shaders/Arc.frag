...
float semiCircleDist(vec2 p, float radius, float angle, float width) {
  width /= 2.0;
  radius -= width;
  return substract(slice(p, angle), abs(circleDist(p, radius)) - width);
}
...
float drawShape(vec2 FragCoord) {
  ...
  //   dist = boxDist(FragCoord, vec2(t_Width, t_Height));
  // } else if (t_Symbol == u_Shapes.Rounded_Rectangle) {
  //   dist = roundBoxDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius, t_Corners);
  // } else if (t_Symbol == u_Shapes.Oval) {
  //   dist = roundBoxDist(FragCoord, vec2(t_Width, t_Height), min(t_Height, t_Width) / 2.0);
  // } else if (t_Symbol == u_Shapes.Chamfered_Rectangle) {
  //   dist = chamferedBoxDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius, t_Corners);
  // } else if (t_Symbol == u_Shapes.Diamond) {
  //   dist = diamonDist(FragCoord, vec2(t_Width, t_Height));
  // } else if (t_Symbol == u_Shapes.Octagon) {
  //   dist = chamferedBoxDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius, 15.0);
  // } else if (t_Symbol == u_Shapes.Round_Donut) {
  //   float InnerCircle = circleDist(FragCoord, t_Inner_Dia / 2.0);
  //   float OuterCircle = circleDist(FragCoord, t_Outer_Dia / 2.0);
  //   dist = substract(InnerCircle, OuterCircle);
  // } else if (t_Symbol == u_Shapes.Square_Donut) {
  //   float InnerSquare = boxDist(FragCoord, vec2(t_Inner_Dia, t_Inner_Dia));
  //   float OuterSquare = boxDist(FragCoord, vec2(t_Outer_Dia, t_Outer_Dia));
  //   dist = substract(InnerSquare, OuterSquare);
  // } else if (t_Symbol == u_Shapes.SquareRound_Donut) {
  //   float InnerCircle = circleDist(FragCoord, t_Inner_Dia / 2.0);
  //   float OuterCircle = boxDist(FragCoord, vec2(t_Outer_Dia, t_Outer_Dia));
  //   dist = substract(InnerCircle, OuterCircle);
  // } else if (t_Symbol == u_Shapes.Rounded_Square_Donut) {
  //   float InnerSquare = roundBoxDist(FragCoord, vec2(t_Inner_Dia, t_Inner_Dia), t_Corner_Radius - (t_Outer_Dia - t_Inner_Dia) / 2.0, t_Corners);
  //   float OuterSquare = roundBoxDist(FragCoord, vec2(t_Outer_Dia, t_Outer_Dia), t_Corner_Radius, t_Corners);
  //   dist = substract(InnerSquare, OuterSquare);
  // } else if (t_Symbol == u_Shapes.Rectange_Donut) {
  //   float InnerRect = boxDist(FragCoord, vec2(t_Width - t_Line_Width * 2.0, t_Height - t_Line_Width * 2.0));
  //   float OuterRect = boxDist(FragCoord, vec2(t_Width, t_Height));
  //   dist = substract(InnerRect, OuterRect);
  // } else if (t_Symbol == u_Shapes.Rounded_Rectangle_Donut) {
  //   float OuterRect = roundBoxDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius, t_Corners);
  //   float InnerRect = roundBoxDist(FragCoord, vec2(t_Width - t_Line_Width * 2.0, t_Height - t_Line_Width * 2.0), t_Corner_Radius - t_Line_Width, t_Corners);
  //   dist = substract(InnerRect, OuterRect);
  // } else if (t_Symbol == u_Shapes.Oval_Donut) {
  //   float OuterOval = roundBoxDist(FragCoord, vec2(t_Width, t_Height), min(t_Height, t_Width) / 2.0);
  //   float InnerOval = roundBoxDist(FragCoord, vec2(t_Width - t_Line_Width * 2.0, t_Height - t_Line_Width * 2.0), min(t_Height, t_Width) / 2.0 - t_Line_Width);
  //   dist = substract(InnerOval, OuterOval);
  // } else if (t_Symbol == u_Shapes.Horizontal_Hexagon) {
  //   dist = horizHexagonDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius);
  // } else if (t_Symbol == u_Shapes.Vertical_Hexagon) {
  //   dist = verticalHexagonDist(FragCoord, vec2(t_Width, t_Height), t_Corner_Radius);
  // } else if (t_Symbol == u_Shapes.Butterfly) {
  //   dist = butterflyDist(FragCoord, t_Outer_Dia / 2.0);
  // } else if (t_Symbol == u_Shapes.Square_Butterfly) {
  //   dist = squareButterflydist(FragCoord, t_Width / 2.0);
  // } else if (t_Symbol == u_Shapes.Triangle) {
  //   dist = triangleDist(FragCoord, t_Width, t_Height);
  // } else if (t_Symbol == u_Shapes.Half_Oval) {
  //   dist = roundBoxDist(FragCoord, vec2(t_Width, t_Height), t_Height / 2.0, 9.0);
  // } else if (t_Symbol == u_Shapes.Rounded_Round_Thermal) {
  //   dist = roundedRoundThermalDist(FragCoord, t_Outer_Dia, t_Inner_Dia, t_Angle, t_Num_Spokes, t_Gap);
  // } else if (t_Symbol == u_Shapes.Squared_Round_Thermal) {
  //   dist = roundThermalDist(FragCoord, t_Outer_Dia, t_Inner_Dia, t_Angle, t_Num_Spokes, t_Gap);
  // } else if (t_Symbol == u_Shapes.Square_Thermal) {
  //   dist = squareThermalDist(FragCoord, t_Outer_Dia, t_Inner_Dia, t_Angle, t_Num_Spokes, t_Gap);
  // } else if (t_Symbol == u_Shapes.Open_Corners_Square_Thermal) {
  //   dist = boxThermalOpenCornersDist(FragCoord, t_Outer_Dia, t_Outer_Dia, t_Angle, t_Num_Spokes, t_Gap, t_Line_Width);
  // } else if (t_Symbol == u_Shapes.Line_Thermal) {
  //   dist = lineThermalDist(FragCoord, t_Outer_Dia, t_Inner_Dia, t_Angle, t_Num_Spokes, t_Gap);
  // } else if (t_Symbol == u_Shapes.Square_Round_Thermal) {
  //   float outerbox = boxDist(FragCoord, vec2(t_Outer_Dia, t_Outer_Dia));
  //   float innercircle = circleDist(FragCoord, t_Inner_Dia / 2.0);
  //   float d = substract(innercircle, outerbox);
  //   float therm = max(d, spokeDist(FragCoord, t_Angle, t_Num_Spokes, t_Gap));
  //   dist = therm;
  // } else if (t_Symbol == u_Shapes.Rectangular_Thermal) {
  //   dist = boxThermalDist(FragCoord, t_Width, t_Height, t_Angle, t_Num_Spokes, t_Gap, t_Line_Width);
  // } else if (t_Symbol == u_Shapes.Rectangular_Thermal_Open_Corners) {
  //   dist = boxThermalOpenCornersDist(FragCoord, t_Width, t_Height, t_Angle, t_Num_Spokes, t_Gap, t_Line_Width);
  // } else if (t_Symbol == u_Shapes.Rounded_Square_Thermal || t_Symbol == u_Shapes.Rounded_Square_Thermal_Open_Corners) {
  //   float OuterRect = roundBoxDist(FragCoord, vec2(t_Outer_Dia, t_Outer_Dia), t_Corner_Radius, t_Corners);
  //   float InnerRect = roundBoxDist(FragCoord, vec2(t_Inner_Dia, t_Inner_Dia), t_Corner_Radius - (t_Outer_Dia - t_Inner_Dia) / 2.0, t_Corners);
  ...
  //   dist = oblongThermalDist(FragCoord, t_Width, t_Height, t_Angle, t_Num_Spokes, t_Gap, t_Line_Width, t_Round);
  // } else if (t_Symbol == u_Shapes.Ellipse) {
  //   dist = ellipseDist(FragCoord, vec2(t_Width / 2.0, t_Height / 2.0));
  // } else if (t_Symbol == u_Shapes.Moire) {
  //   dist = moireDist(FragCoord, t_Ring_Width, t_Ring_Gap, t_Num_Rings, t_Line_Width, t_Line_Length, t_Angle);
  // } else {
  //   dist = -1.0;
  // }
  return dist;
...
void main() {
  ...
  float start = drawShape(translate(FragCoord, (v_Start_Location - v_Center_Location)) * rotateCW(-start_angle));
  float end = drawShape(translate(FragCoord, (v_End_Location - v_Center_Location)) * rotateCW(-end_angle));
