    ...
    var parseBlocks = function() {
        ...
        while(curr.value !== 'EOF') {
            ...
            if(groupIs(0, 'BLOCK')) {
                ...
                ensureHandle(block);
                if(!block.name)
                    console.error('block with handle "' + block.handle + '" is missing a name.');
                else
    ...
    var parseTables = function() {
        ...
        while(curr.value !== 'EOF') {
            ...
            if(groupIs(0, 'TABLE')) {
                ...
                if(tableDefinition) {
                    console.debug(curr.value + ' Table {');
                    tables[tableDefinitions[curr.value].tableName] = parseTable();
                    console.debug('}');
                } else {
                    console.debug('Unhandled Table ' + curr.value);
                }
            } else {
                // else ignored
                curr = scanner.next();
...
function logUnhandledGroup(curr) {
    console.debug('unhandled group ' + debugCode(curr));
}
