...
const GraphicPlotterPrototype: GraphicPlotterImpl = {
  ...
  plot(
    ...
    // ** LINE AND ARC
    if (nextGraphicType === SEGMENT || nextGraphicType === SLOT) {
        if (this._arcDirection === undefined) {
          ...
          const center = getAmbiguousArcCenter(location, this._arcDirection as ArcDirection)
